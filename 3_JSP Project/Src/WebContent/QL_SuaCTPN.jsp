...
<%
 	...
 	session.setAttribute("TenNhaPP", sophieu);
...
<!--Nav-->
	...
	<sql:query dataSource="${snapshot}" var="result">
		SELECT *
		FROM ctphieunhap
		WHERE SoPhieu='<%=session.getAttribute("SoPhieu") %>' and ctphieunhap.MaHang='<%=session.getAttribute("MaHang")%>';
	</sql:query>
...
<div class="container-fluid">
	...
	<div class="col-md-9">
		<h4 class="text-center" style="line-height:40px;color:white; background:gray">Chỉnh Sửa Thông Tin Phiếu Nhập</h4>
	</div>
	<div class="col-md-9">
		
		<form action="suaCTPN.jsp" method="post">
			<c:forEach var="row" items="${result.rows}">			
			
		   	<div class="form-inline">
				<label class="fieldinput col-sm-2">Số Phiếu </label>
				<input type="text" name="SoPhieu" class="form-control" value="${row.SoPhieu}" disabled>
			</div><br>
			<div class="form-inline">
			...
			<div class="form-inline">
				<label class="fieldinput col-sm-2">Số Lượng </label>
					<input type="number" name="SoLuong" class="form-control" value="${row.SoLuongNhap}" required>
				</div><br>
			<div class="form-inline">
